...
var Container = function(containerArray) {
	...
	if (global.config.tlsverify) {
		this.dockerArgs.push('--tlsverify');
	}


	if (global.config.host) {
		this.dockerArgs.push('-H');
		this.dockerArgs.push(global.config.host);
	}
...
Container.prototype.pullImage = function(options, finishedCallback) {
	...
	var pull = spawn('docker', this.dockerArgs.concat(['pull', this.image]), options);
...
Container.prototype.startAndMountContainer = function(options, finishedCallback) {
	var containerId,
		self = this,
		run = spawn('docker', this.dockerArgs.concat(['run', '-d', '-v', global.config.path + ':/app/test', '-w', '/app/test', '-it', self.image, '/bin/bash']));
...
Container.prototype.runBeforeScript = function(options, index, containerId, finishedCallback) {
	...
	spawn('sh', ['-c', 'docker ' + this.dockerArgs.join(' ') + ' exec ' + containerId + ' ' + self.beforeScripts[index]], options).on('exit', function(code) {
		if (0 !== code) {
...
Container.prototype.runTests = function(options, containerId, finishedCallback) {
	...
	var exec = spawn('sh', ['-c', 'docker ' + this.dockerArgs.join(' ') + ' exec ' + containerId + ' bash + -c "source ~/.bashrc && ' + self.testCommand + ' ' + testArgString + ' " 1>&2'], { stdio: 'inherit' }).on('exit', function(code) {
		if (code > 1) {
...
Container.prototype.stopContainer = function(options, containerId, finishedCallback) {
	...
	var stop = spawn('docker', this.dockerArgs.concat(['stop', containerId]), options).on('exit', function(code) {
		if (0 === code) {
			if (global.config.verbose) {
...
Container.prototype.removeContainer = function(options, containerId, finishedCallback) {
	var remove = spawn('docker', this.dockerArgs.concat(['rm', containerId]), options).on('exit', function(code) {
