...
Container.prototype.backupFiles = function(options, containerId, finishedCallback) {
	...
	var exec = spawn('sh', ['-c', 'docker exec ' + containerId + ' ' + this.shell + ' -c "[ -f ~/.bashrc ] && source ~/.bashrc; cp -p -r /app/test /app/backup"']).on('exit', function(code) {
		if (code !== 0) {
			console.error('\nFailed to backup files'.red);
		}
...
Container.prototype.runTests = function(options, containerId, finishedCallback) {
	...
	var exec = spawn('sh', ['-c', 'docker exec ' + containerId + ' ' + this.shell + ' -c "[ -f ~/.bashrc ] && source ~/.bashrc; ' + self.testCommand + ' ' + testArgString + ' " 1>&2'], { stdio: 'inherit' }).on('exit', function(code) {
		if (code !== 0) {
...
Container.prototype.cleanupFiles = function(options, containerId, finishedCallback) {
	...
	var exec = spawn('sh', ['-c', 'docker exec ' + containerId + ' ' + this.shell + ' -c "[ -f ~/.bashrc ] && source ~/.bashrc; rm -rf /app/test/* && cp -p -r /app/backup/* /app/test " 1>&2'], { stdio: 'inherit' }).on('exit', function(code) {
		if (code !== 0) {
